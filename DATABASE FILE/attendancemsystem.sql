...
CREATE TABLE `tblstudents` (
  ...
  `admissionNumber` varchar(255) NOT NULL check (regexp_like(admissionNumber,'AMS[0-9][0-9][0-9]')),
  `password` varchar(50) NOT NULL,
  `classId` varchar(10) NOT NULL,
  `classArmId` varchar(10) NOT NULL,
  `dateCreated` varchar(50) NOT NULL
...
BEFORE INSERT ON tblclassarms
FOR EACH ROW
BEGIN
    DECLARE student_count INT;
    SELECT COUNT(*) INTO student_count FROM tblclassarms WHERE class_id = NEW.class_id AND arm = NEW.arm;
    IF student_count >= 3 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Class arm cannot contain more than 3 students.';
    END IF;
END;

CREATE TRIGGER check_duplicate_admission_number
BEFORE INSERT ON tblstudents
FOR EACH ROW
BEGIN
  IF EXISTS(SELECT * FROM tblstudents WHERE admissionNumber = NEW.admissionNumber) THEN
    SIGNAL SQLSTATE '45000' 
      SET MESSAGE_TEXT = 'This Admission number Already exists!';
  END IF;
END;
